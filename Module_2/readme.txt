...
Ваша задача:
1. Нажать на самую первую строчку в коде на нумерации строк, чтобы поставить точку остановки (breakpoint).
2. Запустить проект нажав на иконку дебага справа от запуска проекта ("жучок").
3. В консоли посмотреть на результат программы.
4. Использовать Step Over для перехода к следующему шагу выполнения программы.
5. Посмотреть на консоль и на ход выполнения программы.
6. Приложить скриншот написанного кода.
...
Пункты задачи:
1. Записать исходный список в переменную my_list.
2. Написать цикл while с соответствующими задаче условиями.
3. Использовать операторы прерывания/продолжения цикла в соответствии с условиями задачи.
...
Пункты задачи:
1. Создать пустые списки primes и not_primes.
2. При помощи цикла for перебрать список numbers.
3. Написать ещё один цикл for (вложенный), где будут подбираться делители для числа из 1ого цикла.
4. Отметить простоту числа можно переменной is_prime, записав в неё занчение True перед проверкой.
5. В процессе проверки на простоту записывать числа из списка numbers в списки primes и not_primes в зависимости от значения переменной is_prime после проверки (True - в prime, False - в not_prime).
6. Вывести списки primes и not_primes на экран (в консоль).
...
Пункты задачи:
1. Объявить функцию get_matrix и написать в ней параметры n, m и value.
2. Создать пустой список matrix внутри функции get_matrix.
3. Написать первый (внешний) цикл for для кол-ва строк матрицы, n повторов.
4. В первом цикле добавлять пустой список в список matrix.
5. Написать второй (внутренний) цикл for для кол-ва столбцов матрицы, m повторов.
6. Во втором цикле пополнять ранее добавленный пустой список значениями value.
7. После всех циклов вернуть значение переменной matrix.
8. Вывести на экран (консоль) результат работы функции get_matrix.
